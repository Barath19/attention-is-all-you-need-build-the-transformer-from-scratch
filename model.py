...
def pad_id_sequence(ids, max_len, pad_id):
    ...
    len_of_ids = len(ids)
    if len_of_ids == max_len: return ids
    elif len_of_ids > max_len:
        return ids[:max_len]
    else:
        pad = [pad_id for _ in range(max_len-len_of_ids)]
        return ids + pad
...
def scaled_dot_product_attention(query, key, value, mask=None):
    ...
    d_k = query.shape[-1]
    raw_score = compute_raw_attention_scores(query, key)
    scores = scale_attention_scores(raw_score, d_k)
...
def merge_heads_back_to_model_dim(multi_head_tensor):
    ...
    return multi_head_tensor.transpose(-2, -3).reshape(B, L, -1)
...
def merge_heads_and_project_output(context, w_o, b_o):
    ...
    merge_head = merge_heads_back_to_model_dim(context)
    return apply_linear_projection(merge_head, w_o, b_o)
...
def assemble_multi_head_attention_forward(query, key, value, w_q, w_k, w_v, w_o, num_heads, mask=None):
    ...
    context, weights = multi_head_scaled_dot_product_attention(qh, kh, vh, mask)
